...
bool Knight::canMoveTo(Square& location) const
{
    ...
    // valid move if moving 1 square forward or backwards
    if (abs(translationY) == 1 && abs(translationX) == 2)
    {
        validMove = true;
    }

    // valid move if moving 1 square sideways and 2 squares forward
    else if (abs(translationX) == 1  && abs(translationX) == 2)
    {
        validMove = true;
    }
